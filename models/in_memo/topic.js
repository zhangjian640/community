...
class Topic {
  constructor(params) {
    if (!params.creator) {
      throw new Error({code: -1, msg:'a topic must be sent by a creator'})
    }
    if (!params.title) {
      throw new Error({code: -1, msg:'a topic must be sent by a title'})
    }

    if (params.content.length < 5) {
      throw new Error({code: -1, msg:"a topic's content must be longer than 5"})
    }
